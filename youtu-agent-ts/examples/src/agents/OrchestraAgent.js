...
const TraceManager_1 = require("../tracing/TraceManager");
class OrchestraAgent extends BaseAgent_1.BaseAgent {
    constructor(config) {
        super(config);
    ...
    async onInitialize() {
        try {
            ...
            if (this.config.plannerModel) {
                this.plannerAgent = await index_1.AgentFactory.createAgent({
                    type: 'simple',
            ...
            if (this.config.reporterModel) {
                this.reporterAgent = await index_1.AgentFactory.createAgent({
                    type: 'simple',
    ...
    async execute(input, recorder) {
        ...
        try {
            ...
            this.traceManager.recordError(traceId, error);
     ...
     * @returns 异步生成器
     */
    async *executeStream(input, recorder) {
     ...
     * @returns 执行结果
     */
    async executeSubtasks(plan, traceId) {
        ...
        for (const subtaskId of plan.executionOrder) {
            ...
            if (!subtask)
                continue;
            // 检查依赖
            const dependenciesMet = subtask.dependencies.every(depId => results.has(depId) && plan.subtasks.find(s => s.id === depId)?.status === 'completed');
            if (!dependenciesMet) {
     ...
     * @returns 执行结果
     */
    async executeSubtask(subtask, traceId) {
        const worker = this.workerAgents.get(subtask.assignedAgent);
        if (!worker) {
            throw new Error(`工作智能体不存在: ${subtask.assignedAgent}`);
        }
        if (!worker.instance) {
            worker.instance = await index_1.AgentFactory.createAgent(worker.config);
        }
        ...
        try {
            ...
            this.traceManager.recordError(traceId, error, { subtask });
            throw error;
        }
    }
    /**
     ...
     * @returns 最终报告
     */
    async report(plan, results, traceId) {
     ...
     * @returns 聚合后的结果
     */
    aggregateResults(plan, _results) {
        let report = `# 任务执行报告\n\n`;
    ...
    buildPlanningPrompt(input) {
        ...
        const workerDescriptions = workers.map(w => `- ${w.name}: ${w.description} (优势: ${w.strengths.join(', ')})`).join('\n');
...
4. 子任务数量控制在3-8个之间`;
     ...
     * @returns 报告提示
     */
    buildReportPrompt(plan, results) {
...
5. 结论和建议`;
     ...
     * @returns 执行计划
     */
    parsePlanningResult(planningResult, originalTask) {
        try {
            // 尝试从结果中提取JSON
            ...
            const subtasks = planData.subtasks.map((task, index) => ({
